class NaturalSciences {
  List collegeRequirementsCourses() => [
        calculus_1_andGeometry,
        calculus_2_andAlgebra,
        mechanics_1,
        mechanics_2,
        computerProgrammingAndApplications,
        generalPhysics_1,
        generalPhysics_2,
        generalChemistry_1,
        generalChemistry_2,
        generalChemistry_Lab_1,
        generalChemistry_Lab_2,
        generalChemistry_3,
      ];
  final List calculus_1_andGeometry = [
    'Calculus (1) and Geometry _ متطلب كلية ',
    '3',
    '040101101',
    [[], []]
  ];
  final List calculus_2_andAlgebra = [
    'Calculus (2) and Algebra _ متطلب كلية ',
    '3',
    '040101102',
    [
      ['040101101'],
      []
    ]
  ];
  final List mechanics_1 = [
    'Mechanics (1) _ متطلب كلية ',
    '2',
    '040101151',
    [[], []]
  ];
  final List mechanics_2 = [
    'Mechanics (2) _ متطلب كلية ',
    '2',
    '040101152',
    [
      ['040101151'],
      []
    ]
  ];
  final List computerProgrammingAndApplications = [
    'Computer Programming and Applications _ متطلب كلية ',
    '2',
    '040102102',
    [[], []]
  ];
  final List generalPhysics_1 = [
    'General Physics (1) _ متطلب كلية ',
    '3',
    '040204101',
    [[], []]
  ];
  final List generalPhysics_2 = [
    'General Physics (2) _ متطلب كلية ',
    '3',
    '040204102',
    [[], []]
  ];
  final List generalChemistry_1 = [
    'General Chemistry (1) _ متطلب كلية ',
    '2',
    '040306101',
    [[], []]
  ];
  final List generalChemistry_2 = [
    'General Chemistry (2) _ متطلب كلية ',
    '2',
    '040306102',
    [[], []]
  ];
  final List generalChemistry_Lab_1 = [
    'General Chemistry Lab (1) _ متطلب كلية ',
    '1',
    '040306103',
    [[], []]
  ];
  final List generalChemistry_Lab_2 = [
    'General Chemistry Lab (2) _ متطلب كلية ',
    '1',
    '040306104',
    [[], []]
  ];
  final List generalChemistry_3 = [
    'General Chemistry (3) _ متطلب كلية ',
    '2',
    '040306106',
    [[], []]
  ];
}
